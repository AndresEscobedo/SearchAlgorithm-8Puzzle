...
void BestFirst::swap_Char(unsigned int indice1, unsigned int indice2, nodo* nodo_actual)
{
...
void BestFirst::devuelve_enlaces_del_nodo(nodo* nodo_actual)
{
...
bool BestFirst::esta_en_ancestros(nodo* actual, char* posible_hijo)
{
...
// devuelve un nodo
void BestFirst::crea_nodo(const char* nombre, nodo* padre, float distancia_al_padre, nodo*& nodo_creado)
{
...
void BestFirst::agrega_hijo(nodo* padre, nodo* hijo)
{
	hijo->padre = padre;
...
bool BestFirst::nodos_iguales_en_nombre(const char* nodo1, const char* nodo2)
{
	return(strcmp(nodo1, nodo2) == 0);
}

// Elimina un elemento de la agenda dado por indice 
void BestFirst::elimina_elemento_agenda(unsigned int indice_a_eliminar)
{
...
void BestFirst::deja_en_la_agenda_solo_los_mejores_k_elementos(unsigned int k)
{
	nodo* temporal;
...
void BestFirst::agrega_un_nodo_al_final_de_la_agenda(nodo* nodo_a_agregar)
{
...
bool BestFirst::busqueda_BestFirst(const char* nodo_inicio, const char* nodo_final)
{
	...
	while (cantidad_de_elementos_en_la_Agenda != 0)
	{
		...
		// Deja todos los elementos en la agenda pero ordenados
		deja_en_la_agenda_solo_los_mejores_k_elementos(cantidad_de_elementos_en_la_Agenda);
	}
	nodo_encontrado_final = NULL;
	return(false);
}

char* BestFirst::devuelve_ruta()
{
