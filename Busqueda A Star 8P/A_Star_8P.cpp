...
Busqueda_pathfinding_a_star::Busqueda_pathfinding_a_star()
{
...
void Busqueda_pathfinding_a_star::swap_Char(unsigned int indice1, unsigned int indice2, nodo* nodo_actual)
{
...
void Busqueda_pathfinding_a_star::devuelve_enlaces_del_nodo(nodo* nodo_actual)
{
	...
	if (indice_X - 3 > 0)
	{
		...
		enlaces_del_nodo[cantidad_de_enlaces_del_nodo]->costo = 1 + calcular_heuristica(enlaces_del_nodo[cantidad_de_enlaces_del_nodo]->matrix);
...
bool Busqueda_pathfinding_a_star::esta_en_ancestros(nodo* actual, char* posible_hijo)
{
...
// devuelve un nodo
void Busqueda_pathfinding_a_star::crea_nodo(const char* nombre, nodo* padre, float distancia_al_padre, nodo*& nodo_creado)
{
...
void Busqueda_pathfinding_a_star::agrega_hijo(nodo* padre, nodo* hijo)
{
	hijo->padre = padre;
...
bool Busqueda_pathfinding_a_star::nodos_iguales_en_nombre(const char* nodo1, const char* nodo2)
{
	return(strcmp(nodo1, nodo2) == 0);
}

// Elimina un elemento de la agenda dado por indice 
void Busqueda_pathfinding_a_star::elimina_elemento_agenda(unsigned int indice_a_eliminar)
{
...
void Busqueda_pathfinding_a_star::deja_en_la_agenda_solo_los_elementos_con_costo_menor(float costo_limite)
{
...
void Busqueda_pathfinding_a_star::agrega_un_nodo_al_final_de_la_agenda(nodo* nodo_a_agregar)
{
	Agenda[cantidad_de_elementos_en_la_Agenda] = nodo_a_agregar;
	cantidad_de_elementos_en_la_Agenda++;
}

// Devuelve la posición en la agenda del nodo de menor costo
unsigned int Busqueda_pathfinding_a_star::devuelve_la_posicion_del_nodo_de_menor_costo_en_la_Agenda()
{
...
float Busqueda_pathfinding_a_star::calcular_heuristica(const char* cadenaActual)
{
	float heuristica = 0;
	for (unsigned int i = 0; i < 9; i++)
	{
		if (cadenaActual[i] != cadenaObjetivo[i])
			heuristica++;
	}
	return heuristica;
}
...
bool Busqueda_pathfinding_a_star::busqueda_pathfinding_a_star(const char* nodo_inicio, const char* nodo_final)
{
	nodo* nodo_actual; // Crea un nodo y lo inicializa
	nodo* nuevo_hijo;
	nodo* meta_temporal = NULL;
	strcpy(cadenaObjetivo, nodo_final);
...
char* Busqueda_pathfinding_a_star::devuelve_ruta()
{
